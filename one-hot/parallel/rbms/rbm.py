...
class RBM:
    ...
    def train(self, train_data, epochs, learning_rate):
        ...
        for epoch in range(epochs):
            ...
            error = self.error(init_visible[606:], curr_visible[606:])            
    ...
    def test(self, test_data, test_answers):
        ...
        for i in range(len(test_data)):
            ...
            error += self.error(visible_answer, visible[606:])
    ...
    def move_locs(self, agent, prey_loc, predator_loc, speed):
        ...
        for i in range(len(locs)):
            ...
            for j in range(2):
                if locs[i][j] >= 0:
                    bin[count * 101 + int(locs[i][j])] = 1
                    count += 1
            input_layer.append(bin)
        input_layer.append(np.zeros(505))
    ...
    def get_allocations_from_binary(self, binary):
        ...
        """
        prey = self.one_hot_to_int(binary[:101])
        agent = self.one_hot_to_int(binary[101:202])
        predator = self.one_hot_to_int(binary[202:])
        return [prey, agent, predator]

    def one_hot_to_int(self, one_hot):
    ...
    def get_location_from_binary(self, binary):
        ...
        """
        x = self.one_hot_to_int(binary[:101])
        y = self.one_hot_to_int(binary[101:])
        return [x, y]
