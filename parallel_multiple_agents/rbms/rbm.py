...
class RBM:
    ...
    def __init__(self, sampler, visible_dim, hidden_dim, cutoff, prey_count, predator_count):
        """
        Parameters
        ----------
        sampler : SamplerRBM
    ...
    def train(self, train_data, epochs, learning_rate):
        ...
        for epoch in range(epochs):
            ...
            error = self.error(init_visible[self.cutoff:], curr_visible[self.cutoff:])            
    ...
    def test(self, test_data, test_answers):
        ...
        for i in range(len(test_data)):
            ...
            error += self.error(visible_answer, visible[self.cutoff:])
    ...
    def move(self, visible_layer):
        ...
        allocs_bin = reconstruction[int(self.cutoff):int(self.cutoff+self.cutoff/2)]
        allocs = self.get_allocations_from_binary(allocs_bin)

        # Get the movement location from binary to decimal
        loc_bin = reconstruction[int(self.cutoff+self.cutoff/2):]
        loc = self.get_location_from_binary(loc_bin)

        return allocs, loc

    def move_locs(self, agent, prey1_loc, prey2_loc, predator1_loc, predator2_loc, speed):
        ...
        locs = [agent.loc, prey1_loc, prey2_loc, predator1_loc, predator2_loc]
        input_layer = []
        ...
        input_layer.append(np.zeros(49))
        ...
        agent.move(prey1_loc, predator1_loc, speed, move_dir)
    ...
    def run_example(self, example, example_answer):
        ...
        # Compute loss
        error = self.error(example_answer, reconstruction[self.cutoff:])
        print("Error: " + str(error))
        print("Ideal allocations: " + str(self.get_allocations_from_binary(example_answer[:int(self.cutoff/2)])))
        print("The model's allocations: " + str(self.get_allocations_from_binary(reconstruction[self.cutoff:int(self.cutoff+self.cutoff/2)])))
        print("Ideal movement location: " + str(self.get_location_from_binary(example_answer[int(self.cutoff/2):])))
        print("The model's movement location: " + str(self.get_location_from_binary(reconstruction[int(self.cutoff+self.cutoff/2):])))
        return
